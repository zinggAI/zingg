...
```bash
spark.master = local[*] spark.executor.memory = 4g spark.driver.memory =
    4g spark.sql.shuffle.partitions = 200
```
...
```bash
./ scripts / zingg.sh-- phase findTrainingData-- conf config.json-- properties -
    file snowEnv.txt
```
