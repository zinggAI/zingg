...
public class TrainingDataFinder extends ZinggBase{
			...
			try{
				...
				if (trFile != null) {
					DataFrame trPairs = DSUtil.joinWithItself(trFile, ColName.CLUSTER_COLUMN, true);
						...
						posPairs = posPairs.drop(Util.toColsArray(ColName.MATCH_FLAG_COL, 
								ColName.COL_PREFIX + ColName.MATCH_FLAG_COL,
								ColName.CLUSTER_COLUMN,
								ColName.COL_PREFIX + ColName.CLUSTER_COLUMN));
						negPairs = negPairs.drop(Util.toColsArray(ColName.MATCH_FLAG_COL, 
								ColName.COL_PREFIX + ColName.MATCH_FLAG_COL,
								ColName.CLUSTER_COLUMN,
								ColName.COL_PREFIX + ColName.CLUSTER_COLUMN));
	...
	public void writeUncertain(DataFrame dupesActual) {
		...
		DataFrame dupes2 = dupes1.sort(Util.toColsArray(col(ColName.CLUSTER_COLUMN)));
	...
	public DataFrame getUncertain(DataFrame dupes) {
		...
		pos = pos.sort(Util.toColsArray(col(ColName.SCORE_COL).asc())).cacheResult();
		if (LOG.isDebugEnabled()) {
		...
		neg = neg.sort(Util.toColsArray(col(ColName.SCORE_COL).desc())).cacheResult();
		if (LOG.isDebugEnabled()) {
