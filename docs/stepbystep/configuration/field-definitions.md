...
The name of the **input column(s)** that populate this field. When a single input column feeds the field, set this to that column name. To combine multiple source columns into one derived field, list them comma-separated — for example, an `address` field built from `address_line1,address_line2,city,state,postal_code`. The output column takes the `fieldName`. In most cases where the field maps one-to-one to a source column, set `fields` to the same value as `fieldName`.
...
|                       DONT_USE | Included in the output but no computation is done on this field. Useful for IDs required in the output. DONT_USE fields are not shown to the labeler when `showConcise` is set to true.                                                                                              | any                                                |
